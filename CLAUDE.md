...
- Auth is OAuth 2.1 + PKCE with no refresh tokens (5-day access token). With the default `SWIGGY_REDIRECT_URI` (http://localhost), `php artisan instamart:login` does DCR + a paste-the-redirect-URL flow. With an HTTPS URI pointing at `/instamart/callback` (must be allowlisted by builders@swiggy.in), the dashboard's **Swiggy login** link (`instamart.login`, `Http\Controllers\SwiggyLoginController`) runs the flow in the browser instead: the PKCE verifier and state live in the session, both routes require the dashboard login, and the command refuses. `SwiggyAuth::clientIdFor()` reuses the saved client only when it was registered for the same redirect URI. Token lives encrypted in `instamart_connections`; a 401 expires it.
